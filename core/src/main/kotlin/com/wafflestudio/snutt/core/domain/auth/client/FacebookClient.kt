...
class FacebookClient(
    ...
    override fun getMe(token: String): OAuth2UserResponse? =
        if (oidcJwtVerifier.looksLikeJwt(token)) getMeFromAuthenticationToken(token) else getMeFromAccessToken(token)

    private fun getMeFromAccessToken(token: String): OAuth2UserResponse {
        val response =
            fetchSocialProfile("facebook") {
                restClient
                    .get()
        ...
        return OAuth2UserResponse(socialId = response.id, name = response.name, email = response.email)
    }
    ...
    private fun getMeFromAuthenticationToken(token: String): OAuth2UserResponse? {
        ...
        return OAuth2UserResponse(
            socialId = claims.subject ?: return null,
